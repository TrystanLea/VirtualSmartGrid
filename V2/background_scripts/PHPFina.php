...
// #### \/ Below are required methods
     ...
     * Create feed
     *
     * @param integer $feedname The id of the feed to be created
     * @param array $options for the engine
    */
    public function create($feedname,$options)
    {
        $interval = (int) $options['interval'];
        if ($interval<5) $interval = 5;
        
        // Check to ensure we dont overwrite an existing feed
        if (!$meta = $this->get_meta($feedname))
        {
            ...
            // Save meta data
            $msg=$this->create_meta($feedname,$meta);
            if ($msg !== true) {
                return $msg;
            }

            $fh = @fopen($this->dir.$feedname.".dat", 'c+');
            if (!$fh) {
            ...
            fclose($fh);
            $this->log->info("create() feedid=$feedname");
        }

        $feedname = "$feedname.meta";
     ...
     * @param integer $feedname The id of the feed to be created
    */
    public function delete($feedname)
    {
        $meta = $this->get_meta($feedname);
        if (!$meta) return false;
        unlink($this->dir.$feedname.".meta");
        unlink($this->dir.$feedname.".dat");
        if (isset($metadata_cache[$feedname])) { unset($metadata_cache[$feedname]); } // Clear static cache
    }
     ...
     * @param integer $feedname The id of the feed to be created
    */
    public function get_meta($feedname)
    {
        $feedname = "$feedname.meta";
        ...
        if (isset($metadata_cache[$feedname])) {
            return $metadata_cache[$feedname]; // Retrieve from static cache
        } else {
            ...
            $metadata_cache[$feedname] = $meta; // Cache it
            return $meta;
        }
    }
     ...
     * @param integer $feedname The id of the feed to be created
    */
    public function get_feed_size($feedname)
    {
        $meta = $this->get_meta($feedname);
        if (!$meta) return false;
        return (16 + filesize($this->dir.$feedname.".dat"));
    }

    /**
     * Adds a data point to the feed
     *
     * @param integer $feedname The id of the feed to add to
     * @param integer $time The unix timestamp of the data point, in seconds
     * @param float $value The value of the data point
     * @param array $arg optional padding mode argument
    */
    public function post($feedname,$timestamp,$value,$padding_mode=null)
    {
        $this->log->info("post() id=$feedname timestamp=$timestamp value=$value padding=$padding_mode");
        ...
        if (!$meta = $this->get_meta($feedname)) {
            $this->log->warn("post() failed to fetch meta id=$feedname");
            return false;
        }
        $meta->npoints = $this->get_npoints($feedname);
        ...
        if ($meta->npoints == 0 && $meta->start_time==0) {
            ...
            $this->create_meta($feedname,$meta);
        }

        if ($timestamp < $meta->start_time) {
            $this->log->warn("post() timestamp older than feed start time id=$feedname");
        ...
        $fh = fopen($this->dir.$feedname.".dat", 'c+');
        if (!$fh) {
            $this->log->warn("post() could not open data file id=$feedname");
            return false;
        }
        ...
        if ($padding>$maxpadding) {
            $this->log->warn("post() padding max block size exeeded id=$feedname, $padding dp");
            return false;
        }
     ...
     * @param float $value The value of the data point
    */
    public function update($feedname,$timestamp,$value)
    {
        if (isset($this->writebuffer[$feedname]) && strlen($this->writebuffer[$feedname]) > 0) {
            $this->post_bulk_save();// if data on buffer, flush buffer now, then update it
            $this->log->info("update() $feedname with buffer");
        }
        return $this->post($feedname,$timestamp,$value);  //post can also update 
    }

    /**
     * Get array with last time and value from a feed
     *
     * @param integer $feedname The id of the feed
    */
    public function lastvalue($feedname)
    {
        $this->log->info("lastvalue() $feedname");
        
        // If meta data file does not exist exit
        if (!$meta = $this->get_meta($feedname)) return false;
        $meta->npoints = $this->get_npoints($feedname);

        if ($meta->npoints>0) {
            $fh = fopen($this->dir.$feedname.".dat", 'rb');
            $size = filesize($this->dir.$feedname.".dat");
            fseek($fh,$size-4);
             $d = fread($fh,4);
            fclose($fh);
     ...
     *
     * @param integer $feedname The id of the feed to fetch from
    ...
    */
    ...
    public function export($feedname,$start)
    {
        $start = (int) $start;
        
        $feedname = $feedname.".dat";
        
        // If meta data file does not exist exit
        if (!$meta = $this->get_meta($feedname)) {
            $this->log->warn("PHPFina:post failed to fetch meta id=$feedname");
            return false;
        }
    ...
    public function csv_export($feedname,$start,$end,$outinterval,$usertimezone)
    {
...
// #### \/ Below are buffer write methods
    ...
    public function post_bulk_prepare($feedname,$timestamp,$value,$padding_mode)
    {   
        $timestamp = (int) $timestamp;
        $value = (float) $value;
        $this->log->info("post_bulk_prepare() feedid=$feedname timestamp=$timestamp value=$value");
        ...
        if (!$meta = $this->get_meta($feedname)) {
            $this->log->warn("post_bulk_prepare() failed to fetch meta feedid=$feedname");
            return false;
        }
        ...
        if ($timestamp < $meta->start_time) {
            $this->log->warn("post_bulk_prepare() timestamp=$timestamp older than feed starttime=$meta->start_time feedid=$feedname");
            return false; // in the past
        }
        ...
        if ($pos>$last_pos) {
            ...
            if (!isset($this->writebuffer[$feedname])) {
                $this->writebuffer[$feedname] = "";    
            }
            ...
            if ($npadding>0) {
                ...
                if ($padding_mode!=null) {
                    ...
                    if (!isset($lastvalue_static_cache[$feedname])) { // Not set, cache it from file data
                        $lastvalue_static_cache[$feedname] = $this->lastvalue($feedname)['value'];
                    }
                    $div = ($value - $lastvalue_static_cache[$feedname]) / ($npadding+1);
                    $padding_value = $lastvalue_static_cache[$feedname];
                    $lastvalue_static_cache[$feedname] = $value; // Set static cache last value
                }
                
                for ($n=0; $n<$npadding; $n++)
                {
                    if ($padding_mode!=null) $padding_value += $div; 
                    $this->writebuffer[$feedname] .= pack("f",$padding_value);
                    //$this->log->info("post_bulk_prepare() ##### paddings ". ((4*$meta->npoints) + (4*$n)) ." $n $padding_mode $padding_value");
                }
            }
            
            $this->writebuffer[$feedname] .= pack("f",$value);
            ...
            // $this->log->warn("post_bulk_prepare() data in past or before next interval, nothing saved. Posting too fast? slot=$meta->interval feedid=$feedname timestamp=$timestamp pos=$pos last_pos=$last_pos value=$value");
        }
    ...
    {
        $byteswritten = 0;
        foreach ($this->writebuffer as $feedname=>$data) {
            ...
            $filename = $this->dir.$feedname.".dat";
...
// #### \/ Bellow are engine private methods    
    ...
    private function create_meta($feedname, $meta)
    {
        $feedname = $feedname . ".meta";
    ...
    public function get_npoints($feedname)
    {
        $bytesize = 0;
        
        if (file_exists($this->dir.$feedname.".dat")) {
            clearstatcache($this->dir.$feedname.".dat");
            $bytesize += filesize($this->dir.$feedname.".dat");
        }
            
        if (isset($this->writebuffer[$feedname]))
            $bytesize += strlen($this->writebuffer[$feedname]);
